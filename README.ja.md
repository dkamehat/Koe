...
> **初回起動 — Windows SmartScreen について。** Koe は未署名で配布しています
> （まだコード署名証明書がありません）。そのため Windows が青い *「Windows によって
> PC が保護されました」* 画面を出すことがあります。**［詳細情報］→［実行］** を
> 押してください。すべてローカルで動作し、どこにも送信されません。
...
### コントロールセンター — すべての機能を1つの窓で開始／停止

Koe には現在3つの機能（ディクテーション・通訳＋字幕・会話）があります。コマンドの
フラグを覚えなくても、次をダブルクリックするだけで各機能を開始／停止できます。

```powershell
.\Koe.bat        # Koe コントロールセンターの窓を開く
```

各行にステータスランプと開始／停止ボタンがあります。各機能は別プロセスで動くので、
1つが落ちても他は生き続けます。窓を閉じると、そこから起動した機能はすべて停止します
（プロセスの取り残しなし）。「初回セットアップをやり直す」でウィザードを再表示、
マイクを使う機能を同時に2つ動かすと警告が出ます。`run.bat` / `run-admin.bat` は
従来どおりディクテーションを直接起動します。
...
Koe Talk は、同じローカル部品を**本当に逐次的な音声会話**に組み上げたものです。
話して、間を置くと、ローカルLLMが声で答え、あなたがまた答える——ターンごとの
キー操作なし、データは一切端末外に出ません（背景の理想は
[docs/VISION.md](docs/VISION.md)）。

```powershell
python talk.py                      # ハンズフリー会話（マイク＋スピーカー）
python talk.py --text               # 声の代わりにタイプ（マイク不要）
python talk.py --role "英会話の練習相手。優しく訂正して"   # ペルソナ
python talk.py --context resume.md --role "面接官"        # 資料に基づく模擬面接
python talk.py --echo-mode headphones   # 話しかけるだけでAIを遮れる
python talk.py --debug              # ターン別レイテンシ計測（gap p50/p95）
```

「音声チャットボット」ではなく「会話」である理由：

- **間を理解する。** ターンの終わりは固定タイムアウトではなく**意味**で決まる：
  「…ですか？」なら約1秒で答え、「…けど」で終わる発話は考え終わるまで待つ
  （`talk_patience` で待ち時間を一括調整）。
- **遮れる。** F8 でいつでも中断——`--echo-mode headphones` なら話し始めるだけで
  AIは即座に黙る。AIは「実際に言えたところまで」しか覚えていない（遮られた人と
  同じ振る舞い）。
- **考え中に話し続けられる。** 返答が始まる前に話し出すと、生成中の返答は破棄され、
  発話は同じターンに合流する（「あ、それと…」が通じる）。
